...
@Service // 해당 클래스 스프링 컨테이너의 빈 등록
public class FileService {

    // [*] 업로드 경로
    // 방법1) 프로젝트내 src : 개발자가 코드를 작성하는 폴더
    // 방법2) 프로젝트내 build : 서버실행했을때 컴파일된 코드 즉] 서버(24시간)의 갖는 실행된 폴더

    // 1. 현재 프로젝트의 최상위 디렉토리(폴더) 경로 찾기
    private String baseDir = System.getProperty("user.dir");
    // 2. 방법2 처럼 개발자폴더가 아닌 실행된 서버의 폴더로 업로드경로 지정하기 , 개발환경에 따라 달라진다.
    ...
    public String fileUpload(MultipartFile multipartFile){
        ...
        // 이유 : uuid 외 파일명 구분을 _언더바 사용하기 위한 내부적인 규칙
        ...
        if( !pathFile.exists()){//.exists() : 지정한 경로가 존재하면 true 존재하지 않으면 false
            pathFile.mkdir(); // .mkdir() : 지정한 경로에 생성 메소드
        }
        //4. 업로드 할 파일의 경로를 file 클래스 생성
        File uploadFile = new File(uploadFilePath);
        //5. 업로드(파일/바이트 이동) 하기, transferTo(file객체) : 지정한 file 객체의 경로로 업로드 파일을 이동
        try{
            multipartFile.transferTo(uploadFile);// 일반예외 발생 multipartFile(업로드된파일)
        }catch (IOException e){
            System.out.println(e);
        } // 6. 만일 업로드 성공시 파일이름 변환하기
        return fileName;
    }
    ...
    public void fileDownload(String fileName, HttpServletResponse response){
        ...
        // 매개변수2 : HttpServletResponse response 다운로드를 욫어한 사용자의 *응답* 객체
        // 1. 다운로드 받을 파일명과 업로드 경로 조합
        String downloadPath = uploadPath +fileName;
        // 2. 만약에 지정한 경로에 파일이 없으면 리턴, File클래스란? 파일/폴더 관련 메소드 제공
        File file = new File( downloadPath);
        if(!file.exists()){return;}
        // 3. 업로드할 파일을 자바(바이트)로 가져오기
        try{ // 3-1 파일 입력스트링 객체 생성
            FileInputStream fin = new FileInputStream( downloadPath);
            // 3-2 파일 용량 만큼 배열 생성 , 읽어온 바이트들을 저장할 배열변수 선언
            long fileSize = file.length(); // 파일 용량/바잍수
            ...
            fin.close(); // 필수는 아니지만 대용량에서는 안전하게 입력스트링을 직접 닫기 권장!
            ...
            // **2. 응답 형식 지정하기 , 다운로드 형식과 다운로드파일지정, URL은 한글 지원하지 않는다.
