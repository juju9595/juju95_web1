...
//AppStrat : 스프링 실행과 동시에 같은패키지/하위패키지내 모든 컴포넌트 스캔 자동 빈(객체) 등록
...
public class RestController1 {
    ...
    @GetMapping("/day03") // HTTP(웹 통신 규약) 중에서 "GET" METHOD 사용, URL 정의 , baseUrl 뒤로 자원URL 정의
    // url : http://localhost:8080/day03
    // Whitelabel Error Page : 요청 정상이지만, 응답은 없다.
    ...
    // ++++ URL? 매개변수명=값1&매개변수명2=값2&매개변수명3=값3       (주의할점 : 문자만 가능)
    ...
    public boolean method2(@RequestParam(name = "name",  defaultValue = "홍길동", required = false) String name){
        ...
        // + @RequestParam(name = "URL매개변수명", defaultValue = "초기값" , required = true/fasle);
        // ++ name = "URL매개변수명" : 주소(URL) 상의 ? 뒤로 매핑(연결)할 매개변수명 명시, 단 자바 매개변수와 이름이 동일시 생략 가능
    ...
    //http://localhost:8080/day03/param2?name=유재석&age=30
    ...
    public String method4(@RequestParam  Map<String, String>map){
            // 일방적으로 정해진 규칙이나 매개변수명이 아닌 단순 MAP(비규칙) 구조일 경우 @RequestParam 사용해야한다.
        System.out.println("RestController1.method4");
        //Map : KEY 와 VALUE 구성에서 한쌍(ENTRY) 구성하여 여러개 ENTRY 저장 구조
        System.out.println("map = " +map);
        return "안녕";//임의값
    }
    @DeleteMapping("/day03/param4")
    // http://localhost:8080/day03/param4?name=유재석&age=40
    // ++DTO 에 자동 매핑하기 위해서는 DTO에도 name 과 age 멤버변수가 존재 해야한다. +생성자+setter
    ...
    /*
    * 1) 쿼리스트링이란 ? URL경로상의 매개변수 표현 , 매개변수 노출o, 보안위험, URL?매개변수=값&매개변수=값
    * 2) BODY(본문)이란 ? HTTP본문에 매개변수 표현, 매개변수 노출x, 보안안전, POST/PUT 에서 JSON타입으로 객체 지원
    * H
             @RequestParam                   vs           @ModelAttribute                       vs      @RequestBody
    역할      단일 파라미터에 변수 바인딩                      복수 파라미터에 객체 바인딩                        본문(body) 객체 바인딩
    생략기준    변수명이 일치할경우(쿼리스트링의매개변수)         DTO 타입일때 기본적용(+생성자,+Setter)             DTO
    주요타입    기본형:int/String/List/Map 자바제공타입;      DTO(개발자가만든객체)                             DTO
    처리가능요청 쿼리스트<form:첨부파일x>                     쿼리스트링, <form:첨부파일o>                       본문(body)
    HTTP메소드  GET/POST/PUT/DELETE                       GET/POST/PUT/DELETE                            POST/PUT
     */

    @PostMapping("/day03/param5")
    // Talend API : [Method] Post, [Scheme] http://localhost:8080/day03/param2
