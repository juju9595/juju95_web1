...
public class LoginController {
    ...
    public boolean login(@RequestBody LoginDto loginDto, HttpServletRequest request){
        ...
        System.out.println(request.getHeader("User-Agent"));// 통신을 요청한 클라이언트(사용자)의 브라우저정보 확인, HTTP Header 확인
        //**** 세션 정보 가져오기 , 세션이란? 톰캣 서버내 저장소 (key-value / MAP 컬레션 사용)
        HttpSession session = request.getSession();
        System.out.println(session.getId());// 세션의 식별 번호 : 브라우저 마다, pc 마다 다르게 할당 된다.
        System.out.println(session.getCreationTime());// 세션의 생성시간(ms) : 최초 요청한 시간
        System.out.println(session.getLastAccessedTime());// 세션의 마지막 접근 시간(ms), ms(밀리초=1000/1 초)
        System.out.println(session.getMaxInactiveInterval());// 세션의 최대 유효 시간(초) 자동 로그아웃 할때와 같은 상황에서 쓰임
        //**** 세션 속성 추가하기, 속성이란? 키와 값으로 구성된 메모리 공간
        session.setAttribute("loginMno", 3);// .setAttribute("속성명", 속성값);
        //loginMno 라는 속성명으로 3 이라는 데이터 저장 하겠다는 뜻
        return true;
    }
    //2. 로그인 된 정보 확인 : 현재 세션에 저장된 회원정보 확인
    @GetMapping("/info")
    public boolean info(HttpServletRequest request){
        //1.요청 서블릿에서 세션정보 꺼내기
        HttpSession session = request.getSession();
        //2. 세션정보내 원하는 속성명 값 꺼내기, 개발자 의도와 상관없이 값은 무조건 Object 타입
