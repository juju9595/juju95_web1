...
const dataAPI = async () =>{
    ...
    let html = '';
        data.data.forEach(value => {
            html +=`<div id="store">
                        <span> 약국명 : ${value.약국명}</span>
                        <span> 전화번호 : ${value.전화번호}</span>
...
//[2] 마커 클러스터러에 클릭이벤트 추가하기
const kakaoMap = async () =>{
    // 지도를 표시할 div // 지도의 중심좌표
     var map = new kakao.maps.Map(document.getElementById('map'), { 
        center : new kakao.maps.LatLng(36.2683, 127.6358), 
        level : 10 // 지도의 확대 레벨
    });

    // (2)마커 클러스터러( 여러개 마커가 겹칠때 도형으로 마커수를 표현 )를 생성합니다
    ...
    //(4) map 반복문을 이용하여 마커를 하나씩 생성하여 return 한 마커를 markers 변수(배열에 대입한다.
