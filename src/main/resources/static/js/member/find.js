...
const findId = async()=>{
    ...
    // 2. 가져온 DOM 객체내 입력받은 값value 가져오기
    const mname = nameInput.value;
    const mphone = phoneInput.value;
    // 3. 선택] body가 아니므로 이름과 연락처를 객체화 + 유효성검사 부가적인
    //FETCH 이용하여 통신 / member/findid?mname=&mphone=
    const response = await fetch(`/member/findid?mname=${mname}&mphone=${mphone}`);
    const data= await response.json();
    // 5-1. 통신한 결과 {msg : 결과메세지 } 출력하기
    alert(data.msg);
}

//[2]
const findPwd = async() =>{
    // 1. HTML 로 부터 아이디 연락처 입력받을 input 마크업 가져오기
    const idInput = document.querySelector("#idInput");
    const phoneInput = document.querySelector("#phoneInput");
    // 2. 가져온 input 마크업에서 입력받은 값 가져오기
    const mid = idInput.value;
    const mphone = phoneInput.value;
    // 3. fetch 이용하여 controller와 통신
